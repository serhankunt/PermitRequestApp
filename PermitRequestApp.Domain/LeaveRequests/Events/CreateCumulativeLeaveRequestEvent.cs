...
public sealed class CreateCumulativeLeaveRequestEvent(
	...
	: INotificationHandler<LeaveRequestDomainEvent>
{
	public async Task Handle(LeaveRequestDomainEvent notification, CancellationToken cancellationToken)
	{
		...
		CumulativeLeaveRequest? cumulativeLeaveRequest = await cumulativeLeaveRequestRepository
			.GetByExpressionAsync(p => p.UserId == leaveRequest.RequestUserId && 
